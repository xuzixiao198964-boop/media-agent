...
在脚本审核通过后、视频生成之前，新增两个评审环节：

```
脚本审核通过（script_approved）
    │
    ▼
┌── 图片提示词评审 ──┐
│  AI 检查每个 scene 的 visual_prompt:        │
│  - 是否准确描述小说场景                     │
│  - 是否包含角色外貌特征                     │
│  - 是否与上下文一致                         │
│  不通过 → AI 自动优化提示词 → 重试(max 3轮) │
└─────────────────────┘
    │ 全部通过
    ▼
┌── AI 图片生成 ──┐
...
| 组件 | 技术 | 说明 |
|------|------|------|
| 提示词评审 | DeepSeek Chat API | 传入小说原文+scene列表，评判提示词质量 |
| 图片评审 | DeepSeek-VL / Gemini Vision | 传入图片+提示词，检查一致性和缺陷 |
...
│  │  PostgreSQL  │                        │
│  └─────────────┘                        │
│  旧服务已停止: ai-novel-agent(9000)       │
│                  media-agent-old(9090)    │
└─────────────────────────────────────────┘
```
...
5. 🆕 图片提示词评审 + 图片评审（v1.1 新增）

### 部署信息
- 服务器: 104.244.90.202:80（v1.1 迁移至80端口）
- 服务状态: 正常运行
- 数据库: PostgreSQL
- 旧服务: ai-novel-agent(9000) 和 media-agent(9090) 已停止